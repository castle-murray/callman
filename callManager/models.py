...
class LaborRequest(models.Model):
    ...
    token = models.UUIDField(default=uuid.uuid4, unique=True, editable=False, blank=True, null=True)
    token_short = models.CharField(max_length=6, blank=True, null=True)
...
class UserProfile(models.Model):
    ...
    slug = models.CharField(max_length=10, unique=True, editable=False, null=True, blank=True)

    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = generate_unique_slug(UserProfile)
        super().save(*args, **kwargs)
    def __str__(self):
